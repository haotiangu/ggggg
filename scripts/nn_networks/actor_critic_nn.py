...
'''
#### this class is for image size 64*64
import torch
import torch.nn as nn
import torch.nn.functional as F
import numpy as np
...
class SmallImageCritic(nn.Module):
	...
	def __init__(self, image_size, state_dim, action_dim):
		...
		self.encoder = nn.Sequential(
			...
			nn.Flatten(), #,
			nn.Linear(64, self.state_dim)  #<--- 784 is hard-coded as dependent on 448 x 448 x 3.    16 is hard-coded as dependent on 224 x 224 x 3.
		)
