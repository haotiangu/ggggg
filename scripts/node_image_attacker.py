...
if __name__ == '__main__':
    ...
    pub_adv_image = rospy.Publisher('/attack_generator_node/perturbation_image', Image, queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        if IMAGE_RECEIVED is not None and TARGET_RECEIVED is not None:
            with torch.no_grad():
                ...
                attacked_obs,X_ATTACKED,adv_image,X_ADV = agent.generate_attack(np_im, act)
                
                #(448, 448, 3) is attacked_obs.shape
                #torch.Size([1, 3, 448, 448]) is X_ATTACKED.shape
                #(448, 448, 3) is adv_image.shape
                #torch.Size([1, 3, 448, 448]) is X_ADV.shape
            ...
            pub_adv_image.publish(adv_frame)
